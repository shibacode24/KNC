<form action="{{ route('grn-store') }}"  method="post" style="background: white">
    @csrf
...
<tr>
    ...
    <input type="hidden" name="material_req_list_id" value="{{ $materialReqListId }}">
        ...
        <div class="col-md-2" style="margin:7px">
            <label class="control-label">Location (Warehouse)<font color="#FF0000">*</font></label>
            <input type="text" class="form-control" value="{{ $warehouse_name->warehouse_name }}" readonly />
            <input type="hidden" name="received_location" value="{{ $warehouse_name->id }}" />
        </div>
