...
@section('content')
                        ...
                        <div class="col-md-12" style="margin-top:15px;">
                            <div class="panel panel-default">
                                <h5 class="panel-title"
                                    ...
                                    align="center">
                                    <i class="fa fa-plus"></i> &nbsp;Direct GRN In Management
                                </h5>
                            </div>
                        </div>
                        <div class="col-md-12" style="margin-top:15px;">
                                        ...
                                        <div class="panel-body tab-content">
                                            <div class="tab-pane active" id="tab8">
                                                <table class="table datatable">
                                                    ...
                                                    <tbody>
                                                        @foreach ($directOrder as $order)
                                                            <tr>
                                                                <td>{{ $loop->iteration }}</td>
                                                                <td>{{ date('d-m-Y',strtotime($order->date)) }}</td>
                                                                <td>{{ $order->order_id }}</td>
                                                                <td>{{ $order->warehouse_name->warehouse_name ?? null }}
                                                                </td>
                                                                ...
                                                                <td>{{ $order->non_consumable_unit_type->unit_type ?? null }}
                                                                </td>
                                                                <td>{{ $order->quantity ?? null }}</td>
                                                                <td>
                                                                    <button data-bs-toggle="modal"
                                                                        data-bs-target="#grn-view-modal"
                                            ...
                                            <div class="tab-pane" id="tab9">
                                                <table class="table datatable">
                                                    ...
                                                    <tbody>
                                                        @foreach ($directCompleteOrder as $completeOrder)
                                                            <tr>
                                                                <td>{{ $loop->iteration }}</td>
    ...
    <div class="modal" id="grn-view-modal" tabindex="-1" role="dialog" aria-labelledby="largeModalHead"
        aria-hidden="true">
...
@section('js')
    <script>
        $(document).off('click', '.grnview').on('click', '.grnview', function() {
            ...
            $.ajax({
                url: '{{ url('non_consumable_viewgrn') }}',
                type: 'get',
                data: {
                    entry_id: entry_id
                },
