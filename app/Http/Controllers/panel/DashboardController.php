...
use Illuminate\Support\Facades\DB;
    ...
    // Status
        ...
        public function nonConsumableCategoryEdit($id){
            $categoryEdit = NonConsumableCategory::find($id);
            return view('adminpanel.non_consumable_category_edit', compact('categoryEdit'));
        }


        public function nonConsumableCategoryUpdate(Request $request)
        {
            // Validate the incoming request data
            $request->validate([
                'category' => 'required|',
            ...
            $category = NonConsumableCategory::find($request->id);
            $category->category = $request->category;

            $category->save();
            return redirect()->route('non-consumable-category')->with('success', 'Category Updated successfully!');
        }
        ...
        public function nonConsumableCategoryMaterialEdit($id){
            $materialEdit = NonConsumableCategoryMaterial::find($id);
            $category = NonConsumableCategory::all();
            $unit = NonConsumableUnitType::all();
            $brand = NonConsumableBrand::all();

            return view('adminpanel.non_consumable_category_material_edit', compact('materialEdit', 'unit', 'category', 'brand'));
        }


        public function nonConsumableCategoryMaterialUpdate(Request $request)
        {
            // Validate the incoming request data
            // $request->validate([
            //     'category' => 'required|',
            ...
            // ]);
            $category = NonConsumableCategoryMaterial::find($request->id);
            $category->category_id = $request->category;
            $category->sub_category_name = $request->sub_category_name;
            $category->brand_id = $request->brand;
            $category->unit_type_id = $request->unit_type;
